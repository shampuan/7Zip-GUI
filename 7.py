...
LANGUAGES = {
    "tr": {
        ...
        "about_text": "7-Zip Arşivleme Arayüzü\n\nVersiyon: 0.1\nGeliştirici: @Zeus github.com/shampuan \nLisans: MIT Lisansı",
        ...
        "default_archive_basename": "yeni_arsiv" # Yeni eklendi
    },
    "en": {
        "window_title": "7Zip GUI",
        "about_title": "About",
        "about_text": "7-Zip Like Archiving Interface\n\nVersion: 0.1\nDeveloper: @Zeus  github.com/shampuan\nLicense: MIT License",
        ...
        "default_archive_basename": "new_archive" # Yeni eklendi
    }
}
...
class SevenZipAddArchiveGUI(QWidget):
    ...
    def _find_image_path(self, filename):
        ...
        possible_paths = [
            ...
            os.path.join(os.path.dirname(os.path.abspath(__file__)), filename), # Betik dizini
            os.path.join("/usr", "share", "bin", "7zip_GUI", "icons", filename) # Kullanıcının belirttiği sistem yolu
        ]
    ...
    def _set_initial_archive_path(self):
        ...
        default_filename = f"{default_basename}.{current_format}"

        # Eğer kaynak dosyalar seçiliyse, ilk kaynak dosyasının dizinini kullan
        if self.selected_source_paths:
            initial_dir = os.path.dirname(self.selected_source_paths[0])
        else:
            # Aksi takdirde, kullanıcının ana dizinini kullan
            initial_dir = os.path.expanduser('~')
        
        self.archive_path_input.setText(os.path.join(initial_dir, default_filename))
    ...
    def add_paths_to_source_list(self, paths):
        lang_data = LANGUAGES[self.current_language]
        current_archive_path = self.archive_path_input.text()
        default_basename = lang_data["default_archive_basename"]
        default_filename_part = f"{default_basename}.{self.format_combo.currentText()}"

        update_archive_path = False
        # Eğer mevcut arşiv yolu varsayılan isme sahipse veya boşsa, güncelle
        # Kullanıcının manuel olarak değiştirip değiştirmediğini anlamak için basit bir kontrol
        if not current_archive_path or (default_filename_part in current_archive_path and os.path.dirname(current_archive_path) == os.path.expanduser('~')):
            update_archive_path = True
        ...
        # Sadece yeni pathler eklendiğinde ve arşiv yolu güncellenecekse
        if update_archive_path and self.selected_source_paths:
            first_source_dir = os.path.dirname(self.selected_source_paths[0])
            self.archive_path_input.setText(os.path.join(first_source_dir, default_filename_part))
    ...
    def browse_archive_path(self):
        lang_data = LANGUAGES[self.current_language]
        current_format = self.format_combo.currentText()
        # Şu anki yoldan dosya adını al, eğer yoksa varsayılanı kullan
        current_path_text = self.archive_path_input.text()
        default_basename = LANGUAGES[self.current_language]["default_archive_basename"]

        if os.path.isdir(current_path_text): # Eğer sadece bir dizin seçiliyse
            initial_dir = current_path_text
            initial_filename = f"{default_basename}.{current_format}"
        elif os.path.isfile(current_path_text): # Eğer bir dosya seçiliyse
            initial_dir = os.path.dirname(current_path_text)
            initial_filename = os.path.basename(current_path_text)
        else: # Varsayılan veya geçersiz bir durumsa
            initial_dir = os.path.expanduser('~')
            initial_filename = f"{default_basename}.{current_format}"
    ...
    def update_archive_extension(self):
        ...
        current_format = self.format_combo.currentText()
        if '.' in current_path:
            base_name = current_path.rsplit('.', 1)[0]
            self.archive_path_input.setText(f"{base_name}.{current_format}")
        else:
            self.archive_path_input.setText(f"{current_path}.{current_format}")
    ...
    def on_ok_clicked(self):
        ...
        # Arşiv yolu (shlex.quote ile güvenli tırnaklama)
        command_parts.append(shlex.quote(archive_path)) 

        # Kaynak yollar (shlex.quote ile güvenli tırnaklama)
        ...
        # Şifre
        if password:
            command_parts.append(f'-p{shlex.quote(password)}') # Şifreyi de tırnak içine al
